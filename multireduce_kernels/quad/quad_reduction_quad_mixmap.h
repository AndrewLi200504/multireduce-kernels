...
template<typename T, T (*Map0)(T), T (*Map1)(T, T), T (*Map2)(T, T), T (*Map3)(T, T),
T (*Op0)(T, T), T (*Op1)(T, T), T (*Op2)(T, T), T (*Op3)(T, T)>
__global__ void quad_reduction_kernel(const T* a, const T* b, const T* c, const T* d, quad_reduction<T>* blockqrs,
    ...
    for (int k = 0; k < STRIDE; k++) {
        ...
        if (currIdx < n) {
            ...
            qr.red2 = Op2(qr.red2, Map2(a[currIdx], c[currIdx]));
            qr.red3 = Op3(qr.red3, Map3(a[currIdx], d[currIdx]));
        } else {
...
template<typename T, T (*Map0)(T), T (*Map1)(T, T), T (*Map2)(T, T), T (*Map3)(T, T), T (*Op0)(T, T), T (*Op1)(T, T), T (*Op2)(T, T),
T (*Op3)(T, T)>
void quad_reduction_launcher(const T* data0, const T* data1, const T* data2, const T* data3, T* out0, T* out1, T* out2, T* out3, int n,
    ...
    quad_reduction_kernel<T, Map0, Map1, Map2, Map3, Op0, Op1, Op2, Op3><<<num_blocks, BLOCK_SIZE>>>(
        data0, data1, data2, data3, device_output, current_n, identity0, identity1, identity2, identity3);
    ...
    while (current_n > BLOCK_SIZE) {
        ...
        quad_reduction_kernel_packed<T, Op0, Op1, Op2, Op3><<<num_blocks, BLOCK_SIZE>>>(
            device_output, device_output, current_n, identity0, identity1, identity2, identity3);

        current_n = num_blocks;
    }

    quad_reduction_kernel_packed<T, Op0, Op1, Op2, Op3><<<1, BLOCK_SIZE>>>(
        device_output, device_output, current_n, identity0, identity1, identity2, identity3);
