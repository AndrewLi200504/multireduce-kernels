...
template<typename T, typename U, U (*Map0)(T, T), U (*Map1)(T, T),
U (*Op0)(U, U), U (*Op1)(U, U)>
__global__ void dual_reduction_kernel(const T* a, const T b, const T c, dual_reduction<U>* blockdrs,
    ...
    for (int k = 0; k < STRIDE; k++) {
        ...
        if (currIdx < n) {
            dr.red0 = Op0(dr.red0, Map0(a[currIdx], b)); 
            dr.red1 = Op1(dr.red1, Map1(a[currIdx], c));
        } else {
...
template<typename T, typename U, U (*Map0)(T, T), U (*Map1)(T, T), U (*Op0)(U, U), U (*Op1)(U, U)>
void dual_reduction_launcher(const T* data, const T b, const T c, U* out0, U* out1, int n, U identity0, 
                            U identity1) {
    int current_n = n;
    ...
    dual_reduction_kernel<T, U, Map0, Map1, Op0, Op1><<<num_blocks, BLOCK_SIZE>>>(
        data, b, c, device_output, current_n, identity0, identity1);
