...
void single_reduction_launcher(const T* data, T* out0, U* out1, int n,
    ...
    single_reduction_kernel_packed<T, U, Cmp, Cmpind><<<1, BLOCK_SIZE>>>(
        intermediate, final_output, current_n, identity, ind_identity);
    cudaFree(intermediate);
