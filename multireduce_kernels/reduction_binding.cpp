...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> acnt_inter_bcnt_binding(torch::Tensor input0, torch::Tensor input1) {
    TORCH_CHECK(input0.is_cuda(), "must be cuda tensor");
    TORCH_CHECK(input1.is_cuda(), "must be cuda tensor");
    TORCH_CHECK(input0.numel() == input1.numel(), "tensors must be the same length");
    auto acnt = torch::empty({1}, input0.options().dtype(torch::kInt));
    auto reduce_and = torch::empty({1}, input0.options().dtype(torch::kInt));
    auto bcnt = torch::empty({1}, input0.options().dtype(torch::kInt));
    acnt_inter_bcnt_launcher(
        input0.data_ptr<bool>(),
        input1.data_ptr<bool>(),
        acnt.data_ptr<int>(),
        reduce_and.data_ptr<int>(),
...
PYBIND11_MODULE(multireduce_kernels, m) {
    ...
    m.def("acnt_inter_bcnt", &acnt_inter_bcnt_binding, R"({"type": "bool", "args": 2, "return_tuple_size": 3, "reds": [{"acnt": 0}, {"intersection": -1}, {"bcnt": 1}]})");
